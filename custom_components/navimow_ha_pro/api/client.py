...
class NavimowCloudClient:
    ...
    def session_state(self) -> dict[str, str]:
        ...
        return {
            ...
            "uid": self._uid,
            "region": self._region,
            "account_region": self._tokens.account_region,
            "host": self._host,
        }
    ...
    def mower_login(self) -> str:
        ...
        Navimow 4.3 has more than one server-side login contract in service
        across its regional clusters. Try a small, deterministic set of known
        layouts. Only the variant label/result is logged; identity and tokens
        are never logged.
        """
        field4 = {
            ...
            "refresh_token": self._tokens.refresh_token,
            # The Android app echoes Passport's exact account-region code here.
            # Host routing remains canonical and independent (ore -> us -> FRA).
            "region": self._tokens.account_region or self._region,
        }
        common_empty = self._common_params(access_token="")
        common_token = self._common_params(access_token=self._tokens.access_token)
        variants = (
            ("app_legacy", {**field4, **common_empty}),
            ("app_access_token", {**field4, **common_token}),
            (
        ...
        for label, body in variants:
            result = self._raw("/user/user/login", body)
            last_result = result
            uid = self._extract_uid(result)
            code = result.get("code") if isinstance(result, dict) else None
            desc = str(result.get("desc", "")) if isinstance(result, dict) else type(result).__name__
            if uid:
                self._uid = str(uid)
                _LOGGER.info(
                    "Navimow mower login accepted host=%s variant=%s code=%s",
                    self._host,
                    label,
                    code,
                )
                return self._uid
            _LOGGER.debug(
                "Navimow mower-login probe rejected host=%s variant=%s code=%s desc=%s",
                self._host,
                label,
                code,
                desc[:160],
        ...
        # Retain the prior signed fallback for older clusters.
        signed = self._add_checkcode({**field4, **common_empty})
        result = self._raw("/user/user/login", signed)
        last_result = result
        uid = self._extract_uid(result)
        code = result.get("code") if isinstance(result, dict) else None
        desc = str(result.get("desc", "")) if isinstance(result, dict) else type(result).__name__
        if uid:
            self._uid = str(uid)
            _LOGGER.info(
                "Navimow mower login accepted host=%s variant=signed_legacy code=%s",
                self._host,
                code,
            )
            return self._uid
        _LOGGER.debug(
            "Navimow mower-login probe rejected host=%s variant=signed_legacy code=%s desc=%s",
            self._host,
            code,
            desc[:160],
        )

        code = last_result.get("code") if isinstance(last_result, dict) else None
        desc = (
            str(last_result.get("desc", ""))
    ...
    def _extract_uid(result: Any) -> str | None:
        ...
        if isinstance(data, dict):
            ...
            if isinstance(data.get("user"), dict):
                containers.append(data["user"])
        for container in containers:
            for key in ("uid", "user_id", "userId"):
                uid = container.get(key)
                if uid:
                    return str(uid)
        return None
    ...
    def iot_file(self, sn: str, file_type: int = 2) -> dict:
        """Return metadata for a mower IoT resource.

        ``file_type=2`` is the LiDAR terrain bundle used by the official app.
        Its response contains the short-lived download URL and map metadata.
        """
        data = self.call(
            "/mowerbot/vehicle/common/get-iot-file",
            {"vehicle_sn": sn, "type": int(file_type)},
        )
