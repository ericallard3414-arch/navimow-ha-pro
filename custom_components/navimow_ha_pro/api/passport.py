...
# Keep this aligned with the currently working official Android client.  The
# value is used both in the signed Passport headers and as ``client_ver`` in
# the private mower-cloud login.  The i2 LiDAR backend returns the otherwise
# opaque business error 5001 when that second login presents the retired
# 4.02.0 client identity.
APP_VERSION = "403000003"
OS_NAME = "Android"
OS_VERSION = "13"
OS_LANGUAGE = "en"
DEVICE = "ANDROID"
...
class Tokens:
    ...
    uuid: str = ""
    region: str = "fra"
    # Exact code returned by Passport (e.g. ``ore``).  This must be echoed to
    # /user/user/login even though host routing uses canonical ``us``.
    account_region: str = ""
...
def _signed_headers(url: str, req_params: dict) -> dict:
    ...
    return {
        ...
        "user-agent": "Segway_Mowerbot/4.3.0 (android)",
    }
...
def _extract_tokens(data: dict) -> Tokens:
    ...
    return Tokens(
        ...
        account_region=str(raw_region or "").strip().lower(),
    )
...
def login(username: str, password: str, region: str | None = None) -> Tokens:
    ...
    for host in passport_hosts(region):
        ...
        if code == _RESULT_OK:
            ...
            if not tokens.account_region:
                tokens.account_region = str(region or tokens.region).strip().lower()
...
def refresh(tokens: Tokens, region: str | None = None) -> Tokens:
    ...
    if not new.account_region:
        new.account_region = tokens.account_region
