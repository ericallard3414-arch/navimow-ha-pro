...
class NavimowLawnMower(CoordinatorEntity[NavimowCoordinator], LawnMowerEntity):
    ...
    async def async_dock(self) -> None:
        """Dock the mower."""
        try:
            # Explicit user intent: clear any older weather/interruption latch
            # before the state transition so RETURN HOME is never mislabelled.
            self.coordinator.mark_manual_return_home()
